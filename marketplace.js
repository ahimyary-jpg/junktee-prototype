...
(() => {
  ...
  function passportLabel(product) { return product.passportEligible === false ? "Passport pending" : "Passport available"; }
  ...
  function productCard(product, compact = false) {
    return `<article class="card product-card marketplace-product ${product.demo ? "is-preview" : ""}" style="width:auto;border:none" data-brand="${e(product.brandId || "junktee")}" data-product="${e(product.id)}" onclick="openProduct('${e(product.id)}')">
      ...
      <button class="brand-link" type="button" onclick="event.stopPropagation();openBrand('${e(product.brandId || "junktee")}')">${e(productBrand(product))}</button>
  ...
  function ensureShell() {
    ...
    document.getElementById("add-to-bag").insertAdjacentHTML("afterend", '<p class="preview-commerce-note">This is a presentation preview. Founder-approved product information, pricing, inventory, and Passport eligibility are required before payment can be enabled.</p>');
    document.getElementById("cabinet-grid")?.closest(".section-tight")?.insertAdjacentHTML("afterend", '<div class="cabinet-brand-filter"><div class="filter-row" id="cabinet-brand-filters"></div></div>');
  }
    ...
    document.getElementById("marketplace-home").innerHTML = `
      ...
      <section class="market-section">
        ...
        <div class="featured-brand-grid">
          ...
          <button class="featured-brand rmayd" type="button" onclick="openBrand('rmayd')"><p class="eyebrow">Confirmed brand · Preview</p><h3>RMAYD</h3><p>Platform structure is live. Approved product and brand details are pending.</p></button>
        </div>
      </section>
      <section class="market-section">
  ...
  function renderBrands() {
    document.getElementById("brands-directory").innerHTML = brands.map((brand, index) => `
      <button class="brand-directory-card ${e(brand.id)}" type="button" onclick="openBrand('${e(brand.id)}')">
        <span class="brand-number">0${index + 1} · ${e(brand.status)}</span><div><h2>${e(brand.name)}</h2><p class="brand-desc">${e(brand.description)}</p></div>
      </button>`).join("");
  }

  function renderCollections() {
    document.getElementById("collections-grid").innerHTML = collections.map((collection) => `
      <button class="collection-tile" type="button" onclick="openCollection('${e(collection.id)}')"><p class="eyebrow">${collection.placeholder ? "Presentation preview" : collection.brandId ? e(brandById(collection.brandId)?.name) : "Cross-brand"}</p><h2>${e(collection.name)}</h2><p>${e(collection.description)}</p></button>`).join("");
  }
  ...
  function renderBrandPage(id) {
    ...
    document.getElementById("brand-page").innerHTML = `
      <header class="brand-hero ${e(brand.id)}"><p class="eyebrow">${e(brand.status)}</p><h1>${e(brand.name)}</h1><p>${e(brand.description)}</p></header>
      <section class="brand-story"><p class="eyebrow">Brand Story</p><div><p class="story-copy">${e(brand.story)}</p>${brand.placeholder ? '<p class="placeholder-disclosure" style="margin-top:28px">Presentation placeholder: no real product name, price, image, inventory, origin, or sustainability claim is being asserted.</p>' : ""}</div></section>
      <section class="market-section" style="border:0;padding-bottom:38px"><div class="market-section-head"><div><p class="eyebrow">${e(brand.name)}</p><h2 class="market-title">${brand.placeholder ? "Preview products" : "Current collection"}</h2></div><button class="btn-text" type="button" onclick="shopBrand('${e(brand.id)}')">Shop this brand →</button></div></section>
      ...
      <section class="market-section platform-passport"><div class="market-section-head"><div><p class="eyebrow" style="color:var(--green-bright)">Passport relationship</p><h2 class="market-title">${brand.passportEligible ? "Eligible pieces enter the shared archive." : "Eligibility is pending product onboarding."}</h2></div><p class="market-copy">The platform records brand and product identity without replacing the label’s own story.</p></div></section>`;
  }
  ...
  function shopMarkup() {
    ...
    document.getElementById("screen-collection").innerHTML = `
      <header class="marketplace-screen-intro"><p class="eyebrow">Independent labels · One destination</p><h1 class="display">Shop</h1><p class="market-copy">Browse the full platform catalog. Preview products are marked clearly and cannot be charged.</p></header>
  ...
  function journalCard(article) {
    ...
    return `<article class="article-card" onclick="${action}"><div class="thumb editorial-placeholder-${e(article.brandId)}"></div><div><p class="eyebrow"><span class="journal-brand">${e(brand)}</span>${e(article.type)}</p><p class="body-md">${e(article.title)}</p><p class="meta">${e(article.time)}${article.placeholder ? " · Editorial pending approval" : ""}</p></div></article>`;
  }
  ...
  function updateProductContext(id) {
    ...
    brandLink.textContent = productBrand(product); brandLink.onclick = () => openBrand(product.brandId || "junktee");
    document.getElementById("pd-price").textContent = product.price || "Price pending";
    document.querySelector("#screen-product .section-tight .meta[style*='color:var(--green)']").textContent = product.purchasable === false ? "Preview · Not available for payment" : "Available in sandbox checkout";
  ...
  function renderCabinetFilters() {
    ...
    wrap.innerHTML = `<button class="pill active" type="button" data-cabinet-brand="all">All Brands</button>${brands.map((brand) => `<button class="pill" type="button" data-cabinet-brand="${e(brand.id)}">${e(brand.name)}</button>`).join("")}`;
    wrap.addEventListener("click", (event) => {
      ...
      document.querySelectorAll("#cabinet-grid .card").forEach((card) => {
        const label = card.querySelector(".brand-link")?.textContent.trim().toLowerCase() || "junktee";
        card.hidden = selected !== "all" && label !== brandById(selected)?.name.toLowerCase();
      });
    });
  }
