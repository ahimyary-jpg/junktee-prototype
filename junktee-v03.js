...
(() => {
  ...
  function isPurchasable(product) {
    return product?.purchasable !== false && priceAmount(product) > 0;
  }

  function displayPrice(product, quantity = 1) {
    return isPurchasable(product) ? money(priceAmount(product) * quantity) : "Price pending";
  }
  ...
  function renderCartV03() {
    ...
    wrap.innerHTML = cart.map((item, index) => `
      <article class="cart-item">
        ...
        <div style="flex:1;">
          <button class="brand-link" type="button" onclick="openBrand('${escapeHTML(item.brandId || "junktee")}')">${escapeHTML(item.brandName || "JUNKTEE")}</button>
  ...
  function renderCheckoutSummary() {
    ...
    itemsWrap.innerHTML = cart.length ? cart.map((item) => `
      <div class="checkout-line">
        <div class="imgbox catalog-product">${productImageHTML(item)}</div>
        <div><button class="brand-link" type="button" onclick="openBrand('${escapeHTML(item.brandId || "junktee")}')">${escapeHTML(item.brandName || "JUNKTEE")}</button><p class="product-name">${escapeHTML(item.name)}</p><p class="meta">Size ${escapeHTML(item.size)} · Qty ${item.quantity}</p></div>
        <p class="price">${displayPrice(item, item.quantity)}</p>
    ...
    const hasPreview = cart.some((item) => !isPurchasable(item));
    document.getElementById("payment-button-total").textContent = money(cartTotal());
    document.getElementById("payment-submit").disabled = !cart.length || hasPreview;
    document.getElementById("payment-submit").title = hasPreview ? "Remove preview items before opening Stripe Test Checkout." : "";
    if (hasPreview) setCheckoutStatus("Preview pieces are saved in your Bag but cannot be charged. Remove them to continue to Stripe Test Checkout.");
    else if (document.getElementById("checkout-status")?.textContent.startsWith("Preview pieces")) clearCheckoutStatus();
  }
  ...
  async function startPayment(event) {
    ...
    if (cart.some((item) => !isPurchasable(item))) {
      setCheckoutStatus("RMAYD preview pieces have no approved price and cannot be sent to Stripe. Remove them to continue with catalog products.", "error");
      return;
    }
